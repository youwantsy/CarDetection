...
def submission(config, file_name, predictions):
    PATH = config.SYSTEM.SAVE_DIR
    TEAM_NAME = config.SYSTEM.TEAM_NAME.replace(" ", "")
...
for epoch in range(1, num_epochs + 1):
    ...
    if loss_valid < loss_min:
        ...
        print("\nMinimum Validation Loss of {:.4f} & Acc of {:.2f} % at epoch {}/{}".format(loss_min, acc_valid * 100.,
                                                                                            epoch, num_epochs))
        print('Model Saved\n')
...
submission(config, test_loader.dataset.images, predictions)